            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    if (!await roleManager.RoleExistsAsync(DS.Admin))
                    {
                        ...
                        await roleManager.CreateAsync(new IdentityRole(DS.Usuario));
                    }

                    var role = Request.Form["radioRol"].ToString();

                    if (role == DS.Admin)
                    {
                        await _userManager.AddToRoleAsync(user,DS.Admin);

                    }else if (role == DS.Usuario)
                    {
                        await _userManager.AddToRoleAsync(user, DS.Usuario);
                    }
                    else
                    {
                        await _userManager.AddToRoleAsync(user, DS.Usuario);
